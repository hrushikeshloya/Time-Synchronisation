...
{cbi(ADCSRA, ADPS2);  // set adc prescale to 4
  ...
  Serial.println("y: "); 
  Serial.write(byte((accelero._offSets[1]+2)>>2));
  Serial.println("z: "); 
  Serial.write(byte((accelero._offSets[2])>>2));
  Serial.flush();
  Serial.end();
  // initialize timer1 
...
void dataISR(){
  if(rising){
  datasend=true;               // send data if rising edge detected on datainterrupt pin
  rising=false;
}
else{rising=true;}
 changes=changes+1;
  if(changes==20){TCNT1=65536-6400;    // sync after 10 cycles of square wave at datainterrupt pin
  synced=true;
  changes=0;}
}
